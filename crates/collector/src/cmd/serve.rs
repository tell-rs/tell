//! Serve command - Run the collector server
//!
...
use cdp_config::{Config, RoutingConfig};
use cdp_protocol::Batch;
use cdp_routing::{RoutingTable, SinkId};
use cdp_sources::tcp::{TcpSource, TcpSourceConfig};
use cdp_sources::tcp_debug::{TcpDebugSource, TcpDebugSourceConfig};
use cdp_sources::{
    ShardedSender, SyslogTcpSource, SyslogTcpSourceConfig, SyslogUdpSource, SyslogUdpSourceConfig,
};
#[cfg(feature = "connector-github")]
use connectors::GitHubConnectorConfig;
#[cfg(feature = "connector-shopify")]
use connectors::ShopifyConnectorConfig;
use connectors::{ConnectorScheduler, ScheduledConnector};
use metrics::{SinkMetricsProvider, SourceMetricsProvider, UnifiedReporter};
use pipeline::{Router, RouterMetricsHandle, SinkHandle};
use sinks::null::NullSink;
use sinks::stdout::{StdoutConfig, StdoutSink};
use tap::{TapPoint, TapServer, TapServerConfig};
...
pub async fn run(args: ServeArgs) -> Result<()> {
    ...
    // Run the collector
    if let Err(e) = run_collector(config).await {
...
async fn run_collector(config: Config) -> Result<()> {
    ...
    let (routing_table, sink_ids) = build_routing_table(&config.routing);
    ...
    info!(
        ...
        "tap server started (use `collector tail` to connect)"
    );
    ...
    // Give sinks time to drain remaining batches
...
/// Registered sink IDs for routing
struct SinkIds {
    null: Option<SinkId>,
    stdout: Option<SinkId>,
}

/// Build routing table from configuration
/// Only registers sinks that are actually referenced in routing config
fn build_routing_table(routing_config: &RoutingConfig) -> (RoutingTable, SinkIds) {
    ...
    // Only register sinks that are referenced
    let mut sink_ids = SinkIds {
        null: None,
        stdout: None,
    };

    if referenced_sinks.contains("null") {
        sink_ids.null = Some(table.register_sink("null"));
    }
    if referenced_sinks.contains("stdout") {
        sink_ids.stdout = Some(table.register_sink("stdout"));
    }
    ...
    let resolve_sink = |name: &str| -> Option<SinkId> {
        match name {
            "null" => sink_ids.null,
            "stdout" => sink_ids.stdout,
            _ => {
                warn!(sink = %name, "unknown sink, skipping");
                None
            }
        }
    };
    ...
    if routing_config.has_default() {
        ...
        // No default configured - use stdout as default
        if let Some(stdout_id) = sink_ids.stdout {
    ...
    for rule in &routing_config.rules {
        ...
        if !sinks.is_empty() {
            ...
            if let Some(ref source_name) = rule.match_condition.source {
                table.add_route(cdp_protocol::SourceId::new(source_name), sinks.clone());
                info!(
                    source = %source_name,
                    sinks = ?rule.sinks,
                    "added routing rule"
            ...
            if let Some(ref source_type) = rule.match_condition.source_type {
                match source_type.as_str() {
                    "tcp" => {
                        // TCP type sources
                        table.add_route(cdp_protocol::SourceId::new("tcp_main"), sinks.clone());
                        table.add_route(cdp_protocol::SourceId::new("tcp_debug"), sinks.clone());
                    }
                    "syslog" => {
                        // Syslog type sources
                        table.add_route(cdp_protocol::SourceId::new("syslog_tcp"), sinks.clone());
                        table.add_route(cdp_protocol::SourceId::new("syslog_udp"), sinks.clone());
                    }
                    _ => {
...
async fn create_and_register_sinks(
    ...
    // Create null sink
    if let Some(null_id) = sink_ids.null {
        let (null_tx, null_rx) = mpsc::channel(config.global.queue_size);
        let null_handle = SinkHandle::new(null_id, "null", null_tx);
        router.register_sink(null_handle);

        let null_sink = NullSink::with_name(null_rx, "null");

        // Collect metrics handle before running
        sink_metrics.push(Arc::new(null_sink.metrics_handle()));

        tasks.push(tokio::spawn(async move {
            let metrics = null_sink.run().await;
            info!(
                sink = "null",
    ...
    // Create stdout sink
    if let Some(stdout_id) = sink_ids.stdout {
        let (stdout_tx, stdout_rx) = mpsc::channel(config.global.queue_size);
        let stdout_handle = SinkHandle::new(stdout_id, "stdout", stdout_tx);
        router.register_sink(stdout_handle);

        let stdout_sink = StdoutSink::with_config(stdout_rx, StdoutConfig::default());

        // Collect metrics handle before running
        sink_metrics.push(Arc::new(stdout_sink.metrics_handle()));

        tasks.push(tokio::spawn(async move {
            let snapshot = stdout_sink.run().await;
            info!(
                sink = "stdout",
...
async fn start_connector_scheduler(
    ...
    for (name, raw_config) in config.connectors.iter() {
        ...
        match raw_config.connector_type.as_str() {
            ...
            other => {
                warn!(connector = %name, connector_type = %other, "unknown connector type, available: {:?}", connectors::available_connectors());
            }
        }
    }
