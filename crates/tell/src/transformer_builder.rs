...
use tell_config::{RoutingConfig, TransformerInstanceConfig};
...
fn get_source_ids_for_rule(rule: &tell_config::RoutingRule) -> Vec<SourceId> {
...
fn build_chain_from_configs(configs: &[TransformerInstanceConfig]) -> Result<Chain> {
    let mut transformers: Vec<Box<dyn tell_transform::Transformer>> = Vec::new();
    ...
    for config in configs {
        ...
        let transformer: Box<dyn tell_transform::Transformer> =
